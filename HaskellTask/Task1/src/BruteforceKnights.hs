...
sort_On f = map snd . sortBy (comparing fst) . map (f &&& id)
...
solve size solution n xy = do
    ...
    let solution'   = M.insert xy n solution
        sortedMoves = sort_On (neighbors size solution) (sequence moves xy)
    if n == size * size
        then [solution']
...
printBoard size solution = board [0..size-1] where
    ...
    separator = intercalate (replicate elemSize '-') (replicate (size + 1) " + ") -- concatenation
